...
const TITLE_OPTIONS = ['Satış Danışmanı', 'Kıdemli Satış Danışmanı', 'Portföy Danışmanı'] as const;
const SPECIALIZATION_OPTIONS = ['Konut', 'Lüks Konut', 'Ticari', 'Kiralama', 'Yatırım'] as const;
...
async function create() {
  ...
  try {
    ...
    const message =
      error instanceof Error ? error.message : t('agents.addError');
...
<template>
  <div v-if="open" class="fixed inset-0 z-[60] flex items-center justify-center p-4">
    ...
    <div class="relative w-full max-w-lg rounded-2xl border border-[#E2E8F0] bg-white p-6 shadow-2xl">
      <h3 class="text-base font-semibold text-[#0A1628]">{{ t('agents.newAgent') }}</h3>
      <p class="mt-1 text-sm text-[#64748B]">{{ t('agents.appearsInSelections') }}</p>
      ...
      <div class="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <div>
          <select
            ...
            class="w-full rounded-lg border border-[#E2E8F0] px-3 py-2.5 text-sm"
          >
            <option value="" disabled>{{ t('agents.selectSpecialization') }}</option>
            <option
          ...
          <p v-if="errors.specialization" class="mt-1 text-xs text-red-500">{{ errors.specialization }}</p>
        </div>
        <div>
          <input
            v-model="form.email"
